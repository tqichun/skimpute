...
train_X = deepcopy(df.iloc[train_ix, :])
train_y = y[train_ix]
test_X = deepcopy(df.iloc[test_ix, :])
test_y = y[test_ix]
imputer = AdaptiveSimpleImputer()
imputer.fit(df)
train_X = imputer.transform(deepcopy(train_X))
test_X = imputer.transform(deepcopy(test_X))
print(train_X)
...
print(score)  # 0.7937219730941704
